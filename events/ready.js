    ...
    async execute(client, commands){
        ...
        var members = await guild.members.fetch()
            ...
            members.forEach((member) => {
                ...
                channels.forEach((channel) => {if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) if(channel.name == channelNumber) return channel.permissionOverwrites.set([{id: member.id, allow: [Permissions.FLAGS.VIEW_CHANNEL]}])})
            })
